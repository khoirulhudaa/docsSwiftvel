...
const Login = () => {
  ...
  const [textError, setTextError] = useState("SIGN UP")
  ...
  return (
    <div className='w-screen h-[100vh] flex items-center justify-center overflow-hidden'>
      ...
      <div className='w-[80%] relative h-[100vh] flex items-center justify-center ml-auto'>
        <div className={`userSI text-[20px] ${textError == 'SIGN UP' ? "bg-indigo-500" : ""} absolute left-[200px] top-[160px] font-normal w-max h-max border rotate-[25deg] border-white rounded-sm text-center text-white px-6 py-3`}>
          SIGN UP
        </div>
        ...
        <div className='w-[44%] h-[70vh] bg-white flex rounded-lg ml-auto flex mr-[150px] shadow-lg flex items-center'>
          ...
          <div className='w-[50%] h-[100%] p-2 overflow-hidden'>
              ...
              <small className='text-[12px] font-normal left-4 top-4 relative'>Already have an account? <span className='text-blue-500 cursor-pointer hover:text-blue-400' onClick={() => navigate('/signIn')}>signin</span></small>
          </div>
        </div>
      </div>
    </div>
